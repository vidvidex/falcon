...
module intt_512_tb;
  ...
  logic [`BRAM_ADDR_WIDTH-1:0] bram2_addr_a, bram2_addr_a_ntt, bram2_addr_a_validate;
  logic [`BRAM_DATA_WIDTH-1:0] bram2_din_a, bram2_dout_a, bram2_dout_a_ntt, bram2_dout_a_validate;
  logic bram2_we_a;
  logic [`BRAM_ADDR_WIDTH-1:0] bram2_addr_b, bram2_addr_b_ntt, bram2_addr_b_validate;
  logic [`BRAM_DATA_WIDTH-1:0] bram2_din_b, bram2_dout_b, bram2_dout_b_ntt, bram2_dout_b_validate;
  logic bram2_we_b;
  bram_6144x128 bram2 (
                  .clka(clk),
                  .addra(bram2_addr_a),
  ...
  ntt #(
        ...
        .bram2_addr_a(bram2_addr_a_ntt),
        .bram2_din_a(bram2_din_a),
        .bram2_dout_a(bram2_dout_a_ntt),
        .bram2_we_a(bram2_we_a),
        .bram2_addr_b(bram2_addr_b_ntt),
        .bram2_din_b(bram2_din_b),
        .bram2_dout_b(bram2_dout_b_ntt),
  ...
  always_ff @(posedge clk) begin
    if(finished_ii === 1 && index < N/2) begin
      if(bram2_dout_a_validate !== expected_polynomial[index])
        $fatal(1, "Test failed at index %d. Expected %d, got %d", index, expected_polynomial[index], bram2_dout_a_validate);
      if(bram2_dout_b_validate !== expected_polynomial[index + N/2])
        $fatal(1, "Test failed at index %d. Expected %d, got %d", index + N/2, expected_polynomial[index + N/2], bram2_dout_b_validate);
      index++;
    end
  ...
  always_comb begin
    if (finished == 1'b0) begin
      bram2_addr_a = bram2_addr_a_ntt;
      bram2_addr_b = bram2_addr_b_ntt;
      bram2_dout_a_ntt = bram2_dout_a;
      bram2_dout_b_ntt = bram2_dout_b;
    end
    else begin
      bram2_addr_a = bram2_addr_a_validate;
      bram2_addr_b = bram2_addr_b_validate;
      bram2_dout_a_validate = bram2_dout_a;
      bram2_dout_b_validate = bram2_dout_b;
    end
  end
  ...
  initial begin
    ...
    finished <= 1;
    bram2_addr_a_validate <= 0;
    bram2_addr_b_validate <= N/2;
    for (int i = 0; i < N/2; i++) begin
      #10;
      bram2_addr_a_validate++;
      bram2_addr_b_validate++;
    end
