...
//
// Because we are never going to use both at the same time, therefore we can save resources by reusing the same module.
//
// Forward NTT: IDLE -> COPY_TO_INT -> NTT -> COPY_FROM_INT -> IDLE
// Inverse NTT: IDLE -> COPY_TO_INT -> NTT -> COPY_FROM_INT -> IDLE
//
...
module ntt_negative#(
  ...
  // Modulo 12289 multiplication (Montgomery multiplication)
  ...
  always_comb begin
    case (state)
      ...
      NTT: begin
        if (butterfly == N/2 && ((mode == 1'b0 && stride == 1) || (mode == 1'b1 && stride == N >> 1)))
      ...
      COPY_FROM_INT: begin
        ...
        for (int i = 0; i < N; i = i + 1) begin
          if(mode == 1'b0)
            ...
            output_polynomial[i] = mod_mult(polynomial[i], intt_scale_factor);  // TODO: there are a lot of multipliers here, maybe do this in multiple clock cycles
        end
      end
    endcase
  end
