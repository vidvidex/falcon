...
//
// Forward NTT: IDLE -> COPY_TO_INT -> NTT <-> NTT_WAIT_FOR_MULTIPLY -> COPY_FROM_INT -> DONE -> IDLE
// Inverse NTT: IDLE -> COPY_TO_INT -> NTT <-> NTT_WAIT_FOR_MULTIPLY -> COPY_FROM_INT -> DONE -> IDLE
//
...
module ntt_negative#(
    ...
    input logic signed [14:0] input_polynomial[N], //! Polynomial in coefficient form

    output logic done, //! NTT is done and output_polynomial is valid
    output logic signed [14:0] output_polynomial[N] //! Polynomial in NTT form
  );

  logic signed [14:0] polynomial[N]; // Intermediate storage for polynomial
  ...
  typedef enum logic [2:0] {
            ...
            COPY_FROM_INT, // Copy intermediate polynomial to output polynomial. For INTT we also scale the coefficients
  ...
  always_comb begin
    ...
    case (state)
      IDLE: begin   // Waiting for the start signal
        if (start == 1'b1)
          next_state = COPY_TO_INT;
      end
      COPY_TO_INT: begin // Copy input polynomial to intermediate polynomial. This takes just one cycle
        next_state = NTT;
      end
      NTT: begin
        if (butterfly == N >> 1)
      ...
      NTT_WAIT_FOR_MULTIPLY: begin // Wait for mod_mult to finish and store the results for this stage.
        // When we get the last product from mod_mult we go to COPY_FROM_INT if we're finished with NTT or to the next stage of NTT if we're not
        if(mod_mult_last == 1'b1)
          if(butterfly - 1 == N >> 1 && ((mode == 1'b0 && stride == 1) || (mode == 1'b1 && stride == N >> 1)))
            next_state = COPY_FROM_INT;
          else
            next_state = NTT;
      end
      COPY_FROM_INT: begin // Copy intermediate polynomial to output polynomial.

        if(mode == 1'b0)  // For NTT we immediately go to DONE
          next_state = DONE;
        else if(mode == 1'b1 && mod_mult_last == 1'b1)  // For INTT we have to scale the coefficients, for which we use a pipeline so we wait for the pipeline to finish
          next_state = DONE;
      end
      DONE: begin
        next_state = IDLE;
  ...
  always_ff @(posedge clk) begin
    ...
    case (state)
      ...
      NTT: begin
        ...
        if (mode == 1'b0) begin
          mod_mult_a <= polynomial[i + stride];
          mod_mult_b <= twiddle_factor;
      ...
      COPY_FROM_INT: begin
        ...
        if(mode == 1'b0)  // For NTT we just copy the intermediate polynomial to the output polynomial
          for (int i = 0; i < N; i = i + 1)
            output_polynomial[i] <= polynomial[i];
        else begin  // For INTT we scale the coefficient using the mod_mult pipeline
          if(intt_scale_index < N) begin
            mod_mult_a <= polynomial[intt_scale_index];
    ...
    // If there is valid output from mod_mult for main NTT operation we store the result in the intermediate polynomial
    ...
    // If there is valid output from mod_mult for main final scaling operation we store the result in the output polynomial
    if(state == COPY_FROM_INT && mod_mult_valid_out == 1'b1)
      output_polynomial[mod_mult_index1_out] <= mod_mult_result;
  end
  ...
  assign twiddle_factor = mode == 1'b0 ? twiddle_rom_ntt[stage + group]:  twiddle_rom_intt[(stage >> 1) + group];
