...
module compress_coefficient(
    ...
    output logic [104:0] compressed_coefficient, //! Compressed coefficient. Only "compressed_coefficient_length" leftmost bits from "compressed_coefficient" are valid
    output logic [6:0] compressed_coefficient_length //! Number of bits used to compress the coefficient.
  );
  ...
  assign compressed_coefficient_length = 1 + 7 + abs_coefficient[13:7] + 1; // 1 sign bit + 7 low bits + high bits (zeros) + 1 high bit (one)
