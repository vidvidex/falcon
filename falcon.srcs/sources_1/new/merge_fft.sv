...
// - f1 (input)
// - f (output)
// Because each of the input is half the size of output this module is designed so that both inputs are in the same BRAM.
// BRAM 1 is always input and BRAM 2 is always output.
// The first half of BRAM 1 is for f0 and the second half for f1. Half in this case refers to "size"/2, not "N"/2
//
//          ---------------------------------------------------------------------------------
// BRAM 1:  |   0   |   1   | ...  | size/4-1     | size/4 | size/4 +1 | ... | size/2-1     |
//          | f0[0] | f0[1] |      | f0[size/4-1] | f1[0]  | f1[1]     |     | f1[size/4-1] |   (total length = "size"/2)
//          ---------------------------------------------------------------------------------
//
//          ---------------------------------------------------------------------------------
// BRAM 2:  |   0   |   1   |   2  |   3   |      ...           | size/2-2    | size/2-1    |
...
module merge_fft#(
    ...
    input logic [$clog2(N):0] size, // Size of input (number of total real and complex values together, size = 1024 means 512 real and 512 imaginary values). Must be <= N/2
    input logic start,
  ...
  assign btf_mode = 1'b0; // For merge_fft we always need mode = 0 (FFT)
  ...
  always_ff @(posedge clk) begin
    if (btf_valid_out) begin
      bram2_addr_a <= u_delayed << 1;
      bram2_addr_b <= (u_delayed << 1) + 1;
