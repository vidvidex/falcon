...
import falconsoar_pkg::*;
  ...
  always_ff @(posedge clk, negedge rst_n) begin
    if     (~rst_n)
      cnt <= '0;
    else if((cnt == 0) && start && en )
      cnt <= 6'd1;
    else if((cnt == 0) && en )
      cnt <= 6'd0;
    else if((done == 1) && en)
      cnt <= 6'd0;
    else if((samp_again == 1) && en)
      cnt <= 6'd0;
    else if(en)
      cnt <= cnt + 1'b1;
  end
  ...
  always_ff @(posedge clk) begin
    if(cnt == 'd08)
      r_floor_flag       <=  fp_sub_data_out[63];
  end
  always_ff @(posedge clk) begin
    if(cnt == 'd17)
      z_floor_flag       <=  fp_sub_data_out[63];
  end
  always_ff @(posedge clk) begin
    if(cnt == 'd18)
      z_final_floor_flag <=  fp_sub_data_out[63];
  end

  always_ff @(posedge clk) begin
    if(cnt == 'd03)
      int_s <= fp_flt2i_data_out;
    else if((cnt == 'd09) && (r_floor_flag))
      int_s <= int_s_redundancy;
  end
  always_ff @(posedge clk) begin
    if(cnt == 'd08)
      fpr_r            <= fp_sub_data_out;
  end
  always_ff @(posedge clk) begin
    if(cnt == 'd12)
      fpr_z            <= fp_mult_data_out;
  end
  always_ff @(posedge clk) begin
    if(cnt == 'd13)
      int_z            <= fp_flt2i_data_out;
  end
  always_ff @(posedge clk) begin
    if(cnt == 'd13)
      int_z_redundancy <= fp_flt2i_data_out - 1;
  end
  always_ff @(posedge clk) begin
    if(cnt == 'd14)
      fpr_z_final      <= fp_mult_data_out;
  end
  always_ff @(posedge clk) begin
    if(cnt == 'd15)
      int_z_final <= fp_flt2i_data_out;
    else if ((cnt == 'd19) && z_final_floor_flag)
      int_z_final <= fp_flt2i_data_out - 1;
    else if ((cnt == 'd39))
      int_z_final <= result_shitf;
  end
  ...
  always_comb begin
    case(cnt)
      6'd00: begin
        fp_mult_data_in_0 = fpr_x;
        fp_mult_data_in_1 = ILN2;
      end
      6'd04: begin
        fp_mult_data_in_0 = fp_i2flt_data_out;
        fp_mult_data_in_1 = LN2;
      end
      6'd05: begin
        fp_mult_data_in_0 = fp_i2flt_data_out;
        fp_mult_data_in_1 = LN2;
      ...
      6'd10: begin
        fp_mult_data_in_0 = fpr_r_sel;
        fp_mult_data_in_1 = TWOPOW63;
      end
      6'd12: begin
        fp_mult_data_in_0 = fpr_ccs;
        fp_mult_data_in_1 = TWOPOW63;
      end
      default: begin
        fp_mult_data_in_0 = 'dx;
        fp_mult_data_in_1 = 'dx;
      end
    endcase
  end
  ///////////////////////////////////////////////////////////////////////////////////////////////
  ...
  always_comb begin
    case(cnt)
      6'd06: begin
        fp_sub_data_in_0 = fpr_x;
        fp_sub_data_in_1 = fp_mult_data_out;
      end
      6'd07: begin
        fp_sub_data_in_0 = fpr_x;
        fp_sub_data_in_1 = fp_mult_data_out;
      ...
      6'd15: begin
        fp_sub_data_in_0 = fpr_z;
        fp_sub_data_in_1 = fp_i2flt_data_out;
      end
      6'd16: begin
        fp_sub_data_in_0 = fpr_z_final;
        fp_sub_data_in_1 = fp_i2flt_data_out;
      end
      default: begin
        fp_sub_data_in_0 = 'dx;
  ...
  always_comb begin
    case(cnt)
      6'd02:
        fp_flt2i_data_in = fp_mult_data_out;
      6'd12:
        fp_flt2i_data_in = fp_mult_data_out;
      6'd14:
        fp_flt2i_data_in = fp_mult_data_out;
  ...
  always_comb begin
    case(cnt)
      6'd03:
        fp_i2flt_data_in = fp_flt2i_data_out;
      6'd04:
        fp_i2flt_data_in = int_s_redundancy;
      6'd14:
        fp_i2flt_data_in = int_z;
      6'd15:
        fp_i2flt_data_in = fp_flt2i_data_out;
      default:
        fp_i2flt_data_in = 'dx;
  ...
  always_ff @(posedge clk) if(en) begin
      if     (cnt == 'd13) begin
        int_y <= C_TABLE[0];
        int_y_redundancy <= C_TABLE[0];
      end
      else if(cnt <  'd38) begin
        int_y <= int_c - products1[126:63];
        int_y_redundancy <= int_c - products1_redundancy[126:63];
      end
    end
  ...
  always_comb begin
    ...
    case(cnt)
      ...
      'd30:
        int_c_pre = C_TABLE[9];
      'd31:
        int_c_pre = C_TABLE[10];
      'd32:
        int_c_pre = C_TABLE[10];
      'd33:
        int_c_pre = C_TABLE[11];
      'd34:
        int_c_pre = C_TABLE[11];
      'd35:
        int_c_pre = C_TABLE[12];
      'd36:
        int_c_pre = C_TABLE[12];
    endcase
  end
  ...
  always_ff @(posedge clk) if(random_req & en)
      uniformbits <= random_bytes;

  assign random_req = (en & ~(done | samp_again)) & ((cnt == 'd38) | ((cnt > 'd38) & (cnt < 'd46) & cmp_equl));

  always_ff @(posedge clk) begin
    if     ((done | samp_again) & en )
      done <= 1'b0; // set 'done' to 1 just one cycle
    else if((cnt > 'd38) & (cnt < 'd47) & cmp_bigger & en)
      done <= 1'b1; // get satisfied value
    else
      done <= 1'b0;
  end
  ...
  always_ff @(posedge clk) begin
    if     ((done | samp_again) & en)
      samp_again <= 1'b0;  // set 'done' to 1 just one cycle
    else if((cnt > 'd38) & (cnt < 'd46) & en & cmp_smaller)
      samp_again <= 1'b1;  // not get satisfied value
    else if((cnt == 'd46) & en & ~cmp_bigger)
      samp_again <= 1'b1;  // not get satisfied value
    else
      samp_again <= 1'b0;
  end
  ...
  always_comb begin
    ...
    case(cnt)
      'd39: begin
        cmp_equl = (uniformbits ==     result_shitf[8*7+:8]);
        cmp_bigger = (uniformbits <     result_shitf[8*7+:8]);
        cmp_smaller = (uniformbits >     result_shitf[8*7+:8]);
      end
      'd40: begin
        cmp_equl = (uniformbits ==      int_z_final[8*6+:8]);
        cmp_bigger = (uniformbits <      int_z_final[8*6+:8]);
        cmp_smaller = (uniformbits >      int_z_final[8*6+:8]);
      end
      'd41: begin
        cmp_equl = (uniformbits ==      int_z_final[8*5+:8]);
        cmp_bigger = (uniformbits <      int_z_final[8*5+:8]);
        cmp_smaller = (uniformbits >      int_z_final[8*5+:8]);
      end
      'd42: begin
        cmp_equl = (uniformbits ==      int_z_final[8*4+:8]);
        cmp_bigger = (uniformbits <      int_z_final[8*4+:8]);
        cmp_smaller = (uniformbits >      int_z_final[8*4+:8]);
      end
      'd43: begin
        cmp_equl = (uniformbits ==      int_z_final[8*3+:8]);
        cmp_bigger = (uniformbits <      int_z_final[8*3+:8]);
        cmp_smaller = (uniformbits >      int_z_final[8*3+:8]);
      end
      'd44: begin
        cmp_equl = (uniformbits ==      int_z_final[8*2+:8]);
        cmp_bigger = (uniformbits <      int_z_final[8*2+:8]);
        cmp_smaller = (uniformbits >      int_z_final[8*2+:8]);
      end
      'd45: begin
        cmp_equl = (uniformbits ==      int_z_final[8*1+:8]);
        cmp_bigger = (uniformbits <      int_z_final[8*1+:8]);
        cmp_smaller = (uniformbits >      int_z_final[8*1+:8]);
      end
      'd46: begin
        cmp_equl = (uniformbits ==      int_z_final[8*0+:8]);
        cmp_bigger = (uniformbits <      int_z_final[8*0+:8]);
        cmp_smaller = (uniformbits >      int_z_final[8*0+:8]);
      end
    endcase
  end
