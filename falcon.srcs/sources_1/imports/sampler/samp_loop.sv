...
import falconsoar_pkg::*;
  ...
  always @(posedge clk) begin
    if ((cnt == 0) && start)
      random_bytes_reg <= random_bytes;
  end

  always @(posedge clk) begin
    if      ((cnt == 0) && en && start)
      int_z0 <=          cmp_result_bitsum;
    else if ((cnt == 1) && en )
      int_z0 <= int_z0 + cmp_result_bitsum;
  end

  always @(posedge clk) begin
    if      ((cnt == 1) && en )
      int_z <=  (random_bytes_reg[72] == 1)? (final_cmp_result_bitsum + 1) : (~(final_cmp_result_bitsum) + 1);
  end
  ...
  always_comb foreach(cmp_result[i]) begin
      case(cnt)
        3'd0:
          cmp_result[i] = (random_bytes[71:0] < RCDT[i]);
        3'd1:
          cmp_result[i] = (random_bytes_reg[71:0] < RCDT[i+9]);
        default:
          cmp_result[i] = 1'b0;
      endcase
    end
  ...
  always_ff @(posedge clk) if(en & ((cnt == 'd3) | (cnt == 'd6)))
      half_isigma_sqr <= cal2samp_loop;

  always_ff @(posedge clk) if(en) begin
      case(cnt)
        'd06:
          fpr_z_sub_r <= fp_sub_data_out; // reg fpr(z) - fpr(r)
        'd09:
          fpr_z_sub_r <= cal2samp_loop;   // reg (z-r)^2
        'd12:
          fpr_z_sub_r <= cal2samp_loop;   // reg (z-r)^2/(2sigma^2)
      endcase
    end
  ...
  always_ff @(posedge clk) if(en) begin
      case(cnt)
        'd07:
          fpr_z0_sqr <= cal2samp_loop; // reg z0^2/(2 sigma_max^2)
      endcase
    end
  ...
  always_ff @(posedge clk) if((cnt == 'd15) & en)
      fpr_x <= fp_sub_data_out;
