...
#(
  ...
  always_ff @(posedge clk) if (base_rdm_req)
      base_rdm144 = {refill_rdm10_l[63:8],refill_rdm10_r[63:8]};
  always_ff @(posedge clk) if (base_rdm_req)
      bef_rdm8_l = refill_rdm10_l[7:0];
  always_ff @(posedge clk) if (base_rdm_req)
      bef_rdm8_r = refill_rdm10_r[7:0];
  always_ff @(posedge clk) if (cmp_rdm_req_l)
      cmp_rdm8_l = refill_rdm1_l;
  always_ff @(posedge clk) if (cmp_rdm_req_r)
      cmp_rdm8_r = refill_rdm1_r;
  ...
  always_comb begin
    case (state)
      INIT:
        next_state = basesampler_done ? PRE : INIT;
      IDLE :
        next_state = (restart & start)? INIT : (start ? PRE : IDLE);
      PRE:
        next_state = (bef_loop_done_l && bef_loop_done_r) ? NREG : PRE;
      NREG:
        next_state = (assist_l || assist_r)? ALOOP : NLOOP;
      NLOOP: begin
        if (!(cmp_done_l && cmp_done_r)) begin
          next_state = NLOOP;
        end
        else if (cmp_rlt_l && cmp_rlt_r) begin
          next_state = F_ADD;
        end
        else if ((!cmp_rlt_l) && (!cmp_rlt_r)) begin
          next_state = NREG;
      ...
      SWITCHL:
        next_state = (bef_loop_done_r)? NREG : SWITCHL;
      SWITCHR:
        next_state = (bef_loop_done_l)? NREG : SWITCHR;
      ALOOP : begin
        if (!(cmp_done_l && cmp_done_r)) begin
          next_state = ALOOP;
        end
      ...
      F_ADD:
        next_state = (final_adder_done)? IDLE : F_ADD;
      default:
        next_state = IDLE;
    endcase
  end
  ...
  always_ff @(posedge clk or negedge reset) begin
    ...
    else if ((state == PRE && pre_state == IDLE) || (state == PRE && pre_state == INIT)) begin
      pre_samp_valid <= 'b1;
    end
    else if (pre_samp_done) begin
      pre_samp_valid <= 'b0;
    end
    else begin
  ...
  always_ff @(posedge clk or negedge reset) begin
    ...
    else if ((state == NLOOP && pre_state == NREG) || (state == ALOOP && pre_state == NREG)) begin
      for_loop_valid_l <= 'b1;
    end
    else if (for_loop_done_l) begin
      for_loop_valid_l <= 'b0;
    end
    else begin
  ...
  always_ff @(posedge clk or negedge reset) begin
    ...
    else if ((pre_samp_done && !assist_l) || (state == SWITCHR && pre_state == NLOOP) || ((state == NLOOP || state ==ALOOP) & basesampler_done)) begin//No sampler_done again
      bef_loop_valid_l <= 'b1;
    end
    else if (bef_loop_done_l) begin
      bef_loop_valid_l <= 'b0;
    end
    else begin
  ...
  always_ff @(posedge clk or negedge reset) begin
    ...
    else if ((pre_samp_done && !assist_r) || (state == SWITCHL && pre_state == NLOOP) || ((state == NLOOP || state ==ALOOP) & basesampler_done)) begin//No sampler_done again
      bef_loop_valid_r <= 'b1;
    end
    else if (bef_loop_done_r) begin
      bef_loop_valid_r <= 'b0;
    end
    else begin
