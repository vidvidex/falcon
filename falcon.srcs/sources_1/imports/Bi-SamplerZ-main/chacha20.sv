...
(
  ...
  always_ff @(posedge clk, negedge rst_n) begin
    if (!rst_n)
      ...
      cnt <= 1;//we can tell that start is a pulse signal
    //else if (start & (restart == 0)) cnt <= 4; start is high only if restart is high, so the condition is forever false
    else if (fetch_en & (buff_idx[4:0] == 'd30))
      cnt <= 7;
    else if (cnt == 0)
      cnt <= 0;
    else if (cnt == 'd88)
      cnt <= 0;//done is a pulse signal.
    else
      cnt <= cnt + 1;
  end
  ...
  always_ff @(posedge clk) begin
    if (start & restart)
      buff_idx <= '0;
    else if(fetch_en & sign_init)
      buff_idx <= buff_idx + 'd4;
    else if(fetch_en & (~sign_init))
