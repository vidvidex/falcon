...
module refill_control
  ...
  always @(posedge clk or negedge rst_n) begin
    if (~rst_n)
      ...
      read_cnt <= 2; // initial fill buffer by using two blocks
    else if (fetch_en & (read_cnt == BLOCK_SIZE))
      read_cnt <= 1; // after read read_cnt + 1
    else if (fetch_en)
      read_cnt <= read_cnt + 1; // after read read_cnt + 1
  end

  always @(*) begin
    if (read_1byte  == 1)
      req_cnt_nxt = (req_cnt + 5'd1);   // to '+1'
    else if ((read_10byte == 1) & (req_cnt >= 23) & (read_cnt == 1))
      req_cnt_nxt = 5'd10;   // to '+10'
    else if (read_10byte == 1)
      req_cnt_nxt = (req_cnt + 5'd10);   // to '+10'
    else
      req_cnt_nxt = req_cnt;
  end
  ...
  always @(posedge clk or negedge rst_n) begin
    if (~rst_n)
      ...
      req_cnt <= 0;
    else if (read_1byte | read_10byte)
      req_cnt <= req_cnt_nxt;
  end
  ...
  always_comb begin
    if(req_cnt <= 'd22) begin
      ...
      case(req_cnt)
        5'd23:
          dout_10 = (read_cnt == 'd1) ? buffer[0*8+:80] : {buffer[0+:8*1],buffer[23*8+:(80-8*1)]};
        5'd24:
          dout_10 = (read_cnt == 'd1) ? buffer[0*8+:80] : {buffer[0+:8*2],buffer[24*8+:(80-8*2)]};
        5'd25:
          dout_10 = (read_cnt == 'd1) ? buffer[0*8+:80] : {buffer[0+:8*3],buffer[25*8+:(80-8*3)]};
        5'd26:
          dout_10 = (read_cnt == 'd1) ? buffer[0*8+:80] : {buffer[0+:8*4],buffer[26*8+:(80-8*4)]};
        5'd27:
          dout_10 = (read_cnt == 'd1) ? buffer[0*8+:80] : {buffer[0+:8*5],buffer[27*8+:(80-8*5)]};
        5'd28:
          dout_10 = (read_cnt == 'd1) ? buffer[0*8+:80] : {buffer[0+:8*6],buffer[28*8+:(80-8*6)]};
        5'd29:
          dout_10 = (read_cnt == 'd1) ? buffer[0*8+:80] : {buffer[0+:8*7],buffer[29*8+:(80-8*7)]};
        5'd30:
          dout_10 = (read_cnt == 'd1) ? buffer[0*8+:80] : {buffer[0+:8*8],buffer[30*8+:(80-8*8)]};
        5'd31:
          dout_10 = (read_cnt == 'd1) ? buffer[0*8+:80] : {buffer[0+:8*9],buffer[31*8+:(80-8*9)]};
      endcase
    end
  end
